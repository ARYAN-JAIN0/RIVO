...
with tab2:
    ...
    try:
        pending_deals = orm_to_df(fetch_pending_deal_reviews())
        ...
        if pending_deals.empty:
            ...
            for _, deal in pending_deals.iterrows():
                with st.container():
                    ...
                    with btn_col1:
                        if st.button("✅ Approve Deal", key=f"sales_approve_{deal['deal_id']}"):
                            mark_deal_decision(deal['deal_id'], "Approved")
                            st.success("Deal approved - moved to Proposal Sent!")
                            st.rerun()
                    
                    with btn_col2:
                        if st.button("❌ Reject Deal", key=f"sales_reject_{deal['deal_id']}"):
...
with tab3:
    ...
    try:
        pending_contracts = orm_to_df(fetch_pending_contract_reviews())
        ...
        if pending_contracts.empty:
            ...
            for _, contract in pending_contracts.iterrows():
                with st.container():
                    ...
                    with col2:
                        st.info(f"💰 ${contract.get('contract_value', 0):,}")
                    ...
                    with btn_col1:
                        if st.button("✅ Approve Strategy", key=f"neg_approve_{contract['contract_id']}"):
                            mark_contract_decision(contract['contract_id'], "Approved")
                            st.success("Strategy approved - contract marked as Signed!")
                            st.rerun()
                    
                    with btn_col2:
                        if st.button("❌ Reject Strategy", key=f"neg_reject_{contract['contract_id']}"):
...
with tab4:
    ...
    try:
        pending_dunning = orm_to_df(fetch_pending_dunning_reviews())
        ...
        if pending_dunning.empty:
            ...
            for _, invoice in pending_dunning.iterrows():
                with st.container():
                    ...
                    with btn_col1:
                        if st.button("✅ Approve & Send", key=f"dunning_approve_{invoice['invoice_id']}"):
                            mark_dunning_decision(invoice['invoice_id'], "Approved")
                            st.success("Dunning message approved!")
                            st.rerun()
                    
                    with btn_col2:
                        if st.button("❌ Reject", key=f"dunning_reject_{invoice['invoice_id']}"):
